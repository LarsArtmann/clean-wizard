...
func (cc *CargoCleaner) Clean(ctx context.Context) result.Result[types.CleanResult] {
	...
	if cc.GetDryRun() {
		...
		if cargoHome != "" {
			registryCache := cargoHome + "/registry"
			if size := cleaner.GetDirSize(registryCache); size > 0 {
				totalBytes += size
				itemsRemoved++
			}

			sourceCache := cargoHome + "/git"
			if size := cleaner.GetDirSize(sourceCache); size > 0 {
				totalBytes += size
				itemsRemoved++
			}
		}
...
func (cc *CargoCleaner) getCargoCacheDir() string {
	...
	if cargoHome == "" {
		if homeDir, err := cleaner.GetHomeDir(); err == nil && homeDir != "" {
