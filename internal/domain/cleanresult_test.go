...
func TestCleanResultValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Invalid CleanResult - negative freed bytes",
			result: CleanResult{
				FreedBytes:   -100,
				ItemsRemoved: 1,
				ItemsFailed:  0,
			...
			shouldValid: false,
			shouldError: true,
			errorMsg:    "FreedBytes cannot be negative, got: -100",
		},
		{
			name: "Invalid CleanResult - negative items removed",
			result: CleanResult{
				FreedBytes:   100,
				ItemsRemoved: -1,
				ItemsFailed:  0,
			...
			shouldValid: false,
			shouldError: true,
			errorMsg:    "ItemsRemoved cannot be negative, got: -1",
		},
		{
